...
class Ion_auth_model extends CI_Model
{
    ...
    public function login($identity, $password, $remember = false)
    {
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, company')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
     ...
     * @author jrmadsen67
     */
    public function get_us_groups($id)
    {
        $existing_group = $this->db->get_where('users_groups', array('user_id' => $id))->row_array();
        return $existing_group['group_id'];
    }

    public function set_session($user)
    {
        $user_group = $this->get_us_groups($user->id);
    ...
    public function login_remembered_user()
    {
        ...
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, email, remember_code, last_login, company')
