...
function create_consumption($data){
...
function create_payment($data){
			$this->db->insert('consumption_account', $data);
			$consumption_id = $this->db->insert_id();
			return $consumption_id;
}
...
function create_customer($data){
	...
	function get_proditem($id){
		$this->db->select('consumption_item.*,material.name');
		$this->db->from('consumption_item');
		$this->db->join('material','consumption_item.material_id=material.id');
		$this->db->where('consumption_item.consumption_id',$id);
		return $this->db->get()->result_array();
		}



function  print_consumption($id){
		...
		$this->db->where('consumption.id',$id);
		$query = $this->db->get();
		return $query->row_array();
	 }


	function  get_allconsumption($branch_id){
		$year_id = $this->get_fyr();
		$this->db->select('consumption.*');
		$this->db->from('consumption');
		//$this->db->join('customer','consumption.customer_id=customer.id');
	 ...
	 function  admin_allconsumption($branch_id){
		$year_id = $this->get_fyr();
		$this->db->select('consumption.*');
		$this->db->from('consumption');
		//$this->db->join('customer','consumption.customer_id=customer.id');
	 ...
	 function get_uncustomerconsumption($code){
		$res = $this->db->query("SELECT consumption.* , customer.name as pnme, customer.contact_no FROM `consumption`,`customer` WHERE consumption.customer_name=customer.code AND consumption.customer_name='$code' ");
	...
	return $customer;
	 ...
	 function get_receipt($code){
		$res = $this->db->query("SELECT consumption.* , customer.name as pnme, customer.contact_no,branch.code as br_code  FROM `consumption`,`customer`,`branch` WHERE  consumption.branch_id=branch.id AND  consumption.customer_name=customer.code AND consumption.id='$code' ");
		$customer = $res->row_array();
		return $customer;
	 }


	function  get_allcustomerconsumption(){
	$all_p = array();
		$res = $this->db->query("SELECT DISTINCT(consumption.customer_name) as p_c, customer.name as pnme FROM `consumption`,`customer` WHERE consumption.customer_name=customer.code");
		$customer = $res->result_array();

		foreach($customer as $pr){

			$res1 = $this->db->query("SELECT SUM(amount) as credit FROM `consumption`  WHERE consumption.type=1 AND consumption.customer_name='{$pr['p_c']}'");
		...
		if($credit['credit'] !=''){
			...
			$res2 = $this->db->query("SELECT SUM(amount) as debit FROM `consumption`  WHERE consumption.type=2 AND consumption.customer_name='{$pr['p_c']}'");
...
function get_branch(){
	...
	{
		$query  = $this->db->get_where('consumption',array('id =' => $id))->row_array();
		return $query;
	}
	...
	{
		$query  = $this->db->get_where('consumption_account',array('id =' => $id))->row_array();
		return $query;
	}
...
public function update_payment($id, $data){
 $res = $this->db->update('consumption_account', $data, array('id' => $id));
 return $res;

}

	 public function update_consumption($id, $data){
	 	$res = $this->db->update('consumption', $data, array('id' => $id));
		return $res;
	...
	public function delete_consumption($id){
			$delete  = $this->db->delete('consumption', array('id' => $id));
			return $delete;
	}
	...
	function get_br($branch_id){
			$sql = "SELECT COUNT(id) AS c_id FROM consumption WHERE branch_id={$branch_id}";
	...
	$sql = "SELECT entry_date FROM `consumption` WHERE `entry_date` > '$start_date' AND  `entry_date` < '$end_date' GROUP BY entry_date";
		$res = $this->db->query($sql);
...
$all_data = array();
	foreach($p_d as $pp){
	$edd= $pp['entry_date'];
	$sql1 = "SELECT SUM(amount) AS credit FROM `consumption` WHERE `entry_date` = '$edd' AND `type`=1";
	$res1 = $this->db->query($sql1);
	$credit = $res1->row_array();


	$sql2 = "SELECT SUM(amount) AS debit FROM `consumption` WHERE `entry_date` = '$edd' AND `type`=2";
...
return $all_data;
	...
	function  get_consumption_report($start_date, $end_date, $type, $customer_id, $year_id){
...
$data['ent'] = date('d-m-Y',strtotime($edt));
	...
	function get_misc_accdetails($dt){
		...
		$this->db->select('consumption.*,consumption.id as s_id,customer.*,branch.code as br_code');
		$this->db->from('consumption');
		$this->db->join('customer','consumption.customer_name = customer.code');
		$this->db->join('branch','consumption.branch_id = branch.id');
		$this->db->where( "consumption.entry_date = '$dt'");
		$this->db->order_by('consumption.id','DESC');
		$query = $this->db->get();
		$s_p =  $query->result_array();
		return $s_p;
