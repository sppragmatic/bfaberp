...
function exportToCSV() {
    ...
    var csv = 'S.No,Name,Code,Address\n';
...
function printTable() {
    ...
    printWindow.document.write('<html><head><title>Product Report</title>');
    ...
    printWindow.document.write('<h2>Product Report - ' + new Date().toLocaleDateString() + '</h2>');
...
$(document).ready(function() {
    var table = $('#product_table').DataTable({
        ...
        "language": {
            ...
            "processing": "Loading product data...",
            "paginate": {
                "first": "«",
                "last": "»", 
                "next": "›",
        ...
        "initComplete": function() {
            if (!$('.table-actions').length) {
                ...
                $('#product_table_wrapper').before(tableActions);
            }
        }
    });
...
<p class="pull-right ">  
   						...
   						<a href="<?= site_url('admin/product/create_product') ?>"><input type="button" value="Create product" class="btn btn-primary"> </a></p>
...
<table class="table table-striped table-bordered table-hover" id="product_table">
									...
									<tbody>
<?php $sm=1; foreach ($product as $fm){?>
<tr>
<td><?php echo $sm++;?></td>
...
<td><a href="<?php echo site_url('admin/product/edit_product/'.$fm->id); ?>">Edit</a> | <a onClick="if(confirm('Are you sure you want to delete this form ?')){location.href='<?php echo site_url('admin/product/delete_product/'.$fm->id);?>';}" style="cursor:pointer;"> Delete</a></td>
</tr>
<?php }?>
